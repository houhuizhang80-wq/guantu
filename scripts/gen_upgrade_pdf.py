...
def build():
    ...
    # 七之二：条线与中局系统
    story.append(P("七之二、晋升条线与中局系统（当前 177 岗）", h1))
    story.append(
        table(
            [
                ["条线", "入口常见", "顶层"],
                ...
                ["发改财政 / 垂管", "县发改、省税务等", "部长 / 署长 / 局长"],
                ["军地交流", "厅局以上事件", "军委主席（登顶分岗）"],
            ],
            [36 * mm, 62 * mm, 72 * mm],
        )
    )
    story.append(Spacer(1, 3 * mm))
    story.append(P("中局怎么不枯燥", h2))
    story.append(
        bullets(
            [
                "政策试点（县处+，经营台）：3–4 月结项，质量进年度考核加分",
                "派系交办：站队后接活；办砸会被对家下月报复",
                "心腹：交往圈好感≥40 结为心腹，可派私事、票决加成",
                "门生：关系页收徒（最多 5 人），交办或盯风声",
                "专项链：暴雨 / 招商 / 巡视三连环；出身长线故事",
                "条线专属事件：法院开庭、纪委谈话室、检察批捕、公安处警等",
                "草率分：连点阶梯累计；档案显示 Y/锁死线；上任大幅清零",
            ]
        )
    )
    story.append(Spacer(1, 3 * mm))
    story.append(
        P(
            "难度参考（512 局稳妥 Bot）：正国约 7%、副国+ 约 22%、省部正+ 约 26%、副部+ 约 63%。"
            "普通玩家多数能到副部，登顶极少。",
            note,
        )
    )
    ...
    qa = [
        ...
        (
            "Q：心腹 / 门生怎么用？",
            "A：NPC 好感≥40 可结心腹并派私事；关系页可收门生交办。信任/忠诚不足会出事。",
        ),
    ]
    for q, a in qa:
