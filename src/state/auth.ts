...
<p>账号使用<strong>邮箱注册与登录</strong>，账号信息与生涯存档保存在 <strong>WorkBuddy 云服务</strong>，可在不同设备上登录同一账号继续游玩。本机浏览器仅保留登录凭据与一份离线缓存，用于断网时继续游玩与加快读取速度。请使用独立密码，不要与其他网站共用。</p>
...
export const GUIDE_STEPS: { title: string; text: string }[] = [
  ...
  {
    title: '存档与账号',
    text: '存档自动进行，不需要手动保存：每一次处置事件、行动、进入下个月之后都会立刻写入你的云账号。本作有 3 个存档槽位，图鉴、成就、出身通关等跨局记录也一并同步，换设备登录同一邮箱即可继续。断网时仍可游玩，存档先留在本机，恢复网络后自动补传。',
  },
]
